...
function AbrirGrupo() {
    connection.on("AbrirGrupo", function (nomeDoGrupo, mensagens) {
        ...
        var mensagemHTML = "";
        for (i = 0; i < mensagens.lenght; i++) {
            mensagemHTML += '<div class="message message-' + (mensagem[i].usuario.id == GetUsuarioLogado().id ? "right" : "left") + '"><div class="message-head"><img src="/imagem/chat.png" />' + mensagem[i].usuario.nome + '</div><div class="message-message">' + mensagem[i].texto + '</div></div>'
         }
    ...
    connection.on("ReceberListaUsuarios", function(usuarios) {
        ...
        for (i = 0; i < usuarios.length; i++) {
            if (usuarios[i].id != GetUsuarioLogado().id) {
                html += '<div class="container-user-item"><img src = "/imagem/logonew.png" style = "width: 10%;" /><div><span>' + usuarios[i].nome.split(" ")[0] + ' (' + (usuarios[i].isOnline ? "online" : "offline") + ')</span><br /><span class="email">' + usuarios[i].email + '</span></div></div>';
            }
        }
